# 信頼区間とは
標本データから母集団の真の値がどの範囲にあるかを幅を持たせて推定する統計手法

## 具体例
- あるLLMモデルを100サンプルで評価した時に正解率が70%だった場合、「真の正解率」=70%とは限らない。
- しかし真の正解率は65~75%と幅を持たせて推定はできる。この65~75%が信頼区間

# 信頼係数とは
- 「この区間推定の方法（例: Wilson区間）で求めた時、その区間が母集団の真の値を含む確率
- よく使われる信頼係数
  - 95%（最も一般的）
  - 99%（より保守的な推定が必要な場合）
  - 90%（探索的分析や予備調査）
- 注意点: 95%信頼区間の意味は「この区間の中に母平均が95%の確率で含まれる」ではなく、「同じ方法で100回標本抽出して信頼区間を計算したら、そのうち約95回は母平均を含む区間が得られる」という意味
- z値は標準正規分布で、中心（平均）から何標準偏差分離れた位置かを表します。
  - z = 0: 平均の位置
  - z = 1: 平均から標準偏差1個分離れた位置
  - z = 1.96: 平均から標準偏差1.96個分離れた位置
